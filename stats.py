...
# Group containers by service type — supports both single-redis and split-redis
def service_group(name):
    # Split Redis instances (new architecture)
    if "redis-bus" in name:
        return "redis-bus"
    if "redis-order" in name:
        return "redis-order"
    if "redis-stock" in name:
        return "redis-stock"
    if "redis-payment" in name:
        return "redis-payment"
    if "redis-orchestrator" in name:
        return "redis-orchestrator"
    # Shared Redis (old architecture)
    if "redis-master" in name:
        return "redis-master"
    if "redis-replica" in name:
        return "redis-replica"
    if "redis-sentinel" in name:
        return "redis-sentinel"
    # App services
    for svc in ["gateway", "orchestrator-worker", "order-web", "order-worker",
                 "stock-web", "stock-worker", "user-web", "user-worker"]:
        if svc in name:
            return svc
    return name
